...
// the risk arrives.
//
//   node tools/check-dco.mjs            # this branch vs origin/main
//   node tools/check-dco.mjs <range>    # any git revision range
...
function revExists(rev) {
  return git(['rev-parse', '--verify', '--quiet', rev]).status === 0;
}
...
function main() {
  ...
  for (const record of records) {
    ...
    if (Date.parse(authoredAt) < ADOPTED) {
      grandfathered += 1;
      continue;
    }
    checked += 1;
