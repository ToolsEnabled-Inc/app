 ...
 * nothing. So this service reads those key NAMES from the engine module rather
 * than restating them, and refuses to create a session that is missing either.
 * A missing pair id is a refusal BEFORE the charge, which costs a signup; the
 * alternative costs a customer.
 *
 * ------------------------------------------------------------------------
 ...
 * merchant's own price ids. Both default under private/, which .gitignore
 * anchors, for the same reason the engine keeps config/entitlement.profile.json
 * out of its tree. Neither has a built-in default value: a MISSING price map is
 * a refusal naming the file, never an empty map that silently sells nothing.
 */
 ...
 */
export function createCheckoutProvider({ mode, secretKey, apiBase, request } = {}) {
  if (mode !== 'test') {
 ...
 */
export function readPriceMap(file = DEFAULT_PRICES) {
  if (!existsSync(file)) {
    throw new SignupRefusal('unavailable',
 ...
 */
export function accountSubscriptionState(account, nowMs) {
 ...
 */
export function createSignupService({ engine, priceMap, provider, store, now, siteOrigin } = {}) {
...
export function createHttpHandler(service, { allowOrigin } = {}) {
...
function loadEngineModel(explicit) {
...
async function main() {
  ...
  const engine = loadEngineModel(argOf('--source'))
  ...
  const server = createServer(createHttpHandler(service, { allowOrigin: argOf('--allow-origin') }))
